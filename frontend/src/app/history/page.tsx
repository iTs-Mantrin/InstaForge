...
import type { HistoryItem, InstagramMediaType } from '@/types';
  ...
  return (
    ...
    >
      <div className="flex gap-4 p-4">
        {/* Thumbnail */}
        <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden flex-shrink-0 bg-surface-800">
            {!imgError ? (
            <Image
              src={item.thumbnail}
              alt={item.title}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-surface-500 mt-0.5">@{item.ownerUsername}</p>
  ...
  const filtered = searchQuery.trim()
    ? items.filter(
        (item) =>
          ...
          item.ownerUsername.toLowerCase().includes(searchQuery.toLowerCase())
      )
    : items;
          ...
          {items.length === 0 && (
            ...
            >
              ...
              <p className="text-sm text-surface-400 mb-6 max-w-sm mx-auto">
                Your download history will appear here after you download your first Instagram content.
              </p>
              <Link
                href="/"
