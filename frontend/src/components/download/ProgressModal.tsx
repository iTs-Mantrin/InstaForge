...
export default function ProgressModal() {
  ...
  useEffect(() => {
    if (isDone && activeJob && mediaInfo && progress?.downloadUrl) {
      addHistoryItem({
        taskId: activeJob.taskId,
        title: mediaInfo.caption?.slice(0, 100) || `Instagram post by @${mediaInfo.ownerUsername}`,
        thumbnail: mediaInfo.displayUrl || mediaInfo.mediaItems[0]?.thumbnail || '',
        url: url,
        mediaType: mediaInfo.mediaType,
        status: 'done',
        downloadUrl: progress.downloadUrl,
        ownerUsername: mediaInfo.ownerUsername,
      });
    }
  }, [isDone, activeJob, mediaInfo, progress, url, addHistoryItem]);
